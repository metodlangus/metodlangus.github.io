const UsefulLinksModule = (() => {
    ...
    let config = {
        ...
        mapPageUrl: '../mattia-adventures-map.html',
        mapTriggerUrl: null
    };
    ...
    function renderLinks() {
        ...
        usefulLinks.forEach(link => {
            html += `<li style="margin-bottom:15px; position:relative;">
                ...
                ${config.mapTriggerUrl && link.url === config.mapTriggerUrl ? `
                    <button class="open-map-btn" data-open-map>🗺️ Zemljevid</button>
                ` : ``}
    ...
    function openMapOverlay() {
        const overlay = document.getElementById(config.mapOverlayId);
        const frame = document.getElementById(config.mapFrameId);

        if (!overlay || !frame) return;

        frame.src = config.mapPageUrl;
    ...
    function init(userConfig = {}) {
        ...
        if (!container) {
            console.warn('UsefulLinksModule: container not found');
            return;
        }
...
window.UsefulLinksModule = UsefulLinksModule;
window.closeMapOverlay = UsefulLinksModule.closeMapOverlay;
