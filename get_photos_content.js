        ...
        .then(data => {
            ...
            if (!entries || entries.length === 0) {
                ...
                const output = uniqueImages
                    .map(img => `${img.imageName} Link: ${img.imgUrl} data-skip=${img.dataSkip} ${img.postUrl}`)
                    .join('\n');
            ...
            entries.forEach(entry => {
                ...
                images.each((i, img) => {
                    ...
                    imageDetailsBuffer.push({ imageName, imgUrl: imgSrc, dataSkip, postUrl });
                });
            });
...
function processDuplicates(imageDetails) {
    ...
    imageDetails.forEach(({ imageName, imgUrl, dataSkip, postUrl }) => {
        if (!filenameCounts[imageName]) {
            filenameCounts[imageName] = { count: 0, urls: [], dataSkips: [], postUrls: [] };
        }
        ...
        if (filenameCounts[imageName].count === 1) {
            uniqueImages.push({ imageName, imgUrl, dataSkip, postUrl });
        }
    });
